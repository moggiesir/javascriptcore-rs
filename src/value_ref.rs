...
impl ValueRef {
    ...
    pub fn to_number(&self, context: &GlobalContextRef) -> Option<f64> {
        ...
        let result = unsafe { JSValueToNumber(context.to_glib_none().0, unwrap(self.to_glib_none().0), &mut exception) };
    ...
    pub fn to_string(&self, context: &GlobalContextRef) -> Option<String> {
        unsafe {
            let mut exception = std::ptr::null_mut();
            let jsstring = JSValueToStringCopy(context.to_glib_none().0, unwrap(self.to_glib_none().0), &mut exception);
...
impl FromGlibPtrNone<JSValueRef> for ValueRef {
    unsafe fn from_glib_none(ptr: JSValueRef) -> Self {
        let mut wrapper = RefWrapper {context: std::ptr::null_mut() as _, value: ptr};
