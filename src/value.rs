...
impl AsJsValue for Vec<&dyn AsJsValue> {
    fn as_js_value(&self, context: &Context) -> Value {
        Value::new_array(context, self)
    }
}

impl AsJsValue for HashMap<&str, &dyn AsJsValue> {
    fn as_js_value(&self, context: &Context) -> Value {
...
impl Value {
    pub fn new_array<P: IsA<Context>>(context: &P, array: &Vec<&dyn AsJsValue>) -> Value {
        unsafe {
            let free_fn: unsafe extern "C" fn(*mut gobject_sys::GObject) =
                gobject_sys::g_object_unref as _;
            let free_ptr: *const _ = &free_fn as _;
            let converted: *const unsafe extern "C" fn(glib_sys::gpointer) = free_ptr as _;
            let garray =
                glib_sys::g_ptr_array_new_full(array.len().try_into().unwrap(), Some(*converted));
            for item in array {
            ...
            let result = from_glib_full(javascriptcore_sys::jsc_value_new_array_from_garray(
    ...
    pub fn new_function<P: IsA<Context>, Q: Fn(Vec<Value>) -> Value + 'static>(
        context: &P,
        name: Option<&str>,
        callback: Q,
        return_type: glib::types::Type,
    ) -> Value {
        skip_assert_initialized!();
        let callback_data: Box<Q> = Box::new(callback);
        unsafe extern "C" fn callback_func<
            P: IsA<Context>,
            Q: Fn(Vec<Value>) -> Value + 'static,
        >(
            args: *mut glib_sys::GPtrArray,
        ...
        unsafe {
            from_glib_full(javascriptcore_sys::jsc_value_new_function_variadic(
                context.as_ref().to_glib_none().0,
                name.to_glib_none().0,
                callback,
    ...
    pub fn new_object<T, P: IsA<Context>>(
        context: &P,
...
pub trait ValueExtManual: 'static {
    fn object_define_property_accessor(
        &self,
        property_name: &str,
        flags: ValuePropertyFlags,
        property_type: glib::types::Type,
        getter: Option<Box<dyn Fn() -> Value + 'static>>,
        setter: Option<Box<dyn Fn(&Value) -> Value + 'static>>,
    );
}
...
impl<O: IsA<Value>> ValueExtManual for O {
    fn object_define_property_accessor(
        ...
        unsafe extern "C" fn getter_func(
            user_data: glib_sys::gpointer,
        ) -> *mut javascriptcore_sys::JSCValue {
            let callback: &Box_<(
                &Option<Box<dyn Fn() -> Value + 'static>>,
                &Option<Box<dyn Fn(&Value) -> Value + 'static>>,
            )> = &*(user_data as *mut _);
        ...
        let super_callback0: Box_<(
            &Option<Box<dyn Fn() -> Value + 'static>>,
            &Option<Box<dyn Fn(&Value) -> Value + 'static>>,
        )> = Box_::new((&getter_data, &setter_data));
        unsafe {
            javascriptcore_sys::jsc_value_object_define_property_accessor(
                self.as_ref().to_glib_none().0,
                property_name.to_glib_none().0,
                flags.to_glib(),
                property_type.to_glib(),
                getter,
                setter,
                Box::into_raw(super_callback0) as *mut _,
                destroy_call7,
...
mod tests {
    ...
    fn new_array() {
        ...
        assert_eq!(
            ValueExt::to_string(&ctx.evaluate("myArray.join(' : ')").unwrap()).as_str(),
